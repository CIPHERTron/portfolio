...
type IContentType = 'articles' | 'work';
...
export const getContentData = async (id: string, contentType: IContentType) => {
  ...
  const processedContent = await remark().use(html).use(remarkPrism).process(matterResult.content);
 ...
 * @param {string} tag - tag to filter by
 */
export const getContentInCategory = (category: string, contentType: IContentType) => {
  let contentDir;
  let contentFiles;
